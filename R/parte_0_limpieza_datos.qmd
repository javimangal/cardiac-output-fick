...
overview_na(data %>% select(all_of(ventilation_variables)))
```
...
overview_na(
  data %>% 
    select(all_of(ventilation_variables)) %>%  
...
data <- data %>% 
  ...
  relocate(c(CI_TTE_1, CI_TTE_2, CI_TTE_3), .before = cardiac_index_TTE) %>% 
...
write.table(
  data,
  file = file.path(psfolder, "cardiac_output_Fick_vs_ECOTT.tab"),
  sep = "\t",         
  quote = FALSE,      
  row.names = FALSE,  
  na = ""             
