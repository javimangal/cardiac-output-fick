...
local_file <- paste0(tempfolder,"/cardiac_index_Fick_vs_ECOTT.RData")
...
```{r}
# Perform bootstrapping for cardiac_index_Fick
set.seed(123)  # For reproducibility
...
The mean cardiac index for TTE is `r mean(data$cardiac_index_TTE) %>% round(2)` L/min (95% CI: `r ci_bca_mean_CI_TTE$bca[4] %>% round(2)` to `r ci_bca_mean_CI_TTE$bca[5] %>% round(2)`), and for Fick, `r mean(data$cardiac_index_Fick) %>% round(2)` L/min (95% CI: `r ci_bca_mean_CI_Fick$bca[4] %>% round(2)` to `r ci_bca_mean_CI_Fick$bca[5] %>% round(2)`).
...
Would it be relevant to study the effect of the observer on the cardiac index measurements?

```{r}
boxplot(
  cardiac_index_TTE ~ observer,
  data = data,
  main = "TTE Cardiac Index by Observer"
  )
```
...
linear_mixed_model_obs_fixed <- lmer(
  cardiac_index_Fick ~ cardiac_index_TTE + observer + (1 | ID) + (1 | time_point),
  data = data
)
...
linear_mixed_model_obs_random <- lmer(
  cardiac_index_Fick ~ cardiac_index_TTE + (1 | ID) + (1 | time_point) + (1 | observer),
  data = data
)
...
data %>% 
  ...
  gt %>% 
  cols_label(
    CI_TTE_1 = "CI 1 (TTE), L/min",
    CI_TTE_2 = "CI 2 (TTE), L/min",
    CI_TTE_3 = "CI 3 (TTE), L/min",
    cardiac_index_TTE = "Mean CI TTE, L/min",
    CV_TTE_CI_row = "CV (%)",
    CE_TTE_CI = "CE (%)",
    cardiac_index_Fick = "CI (Fick method), L/min"
  ) %>% 
  # Adjust column sizes
  cols_width(
    ID ~ pct(10),
...
As before, the model with observer not significantly better, but the distribution of observations improves by including it. Thus, will include. 
...
fig2B <- ggplot(data_long, aes(x = mean_values, y = differences, color = TTE_measurement)) +
  ...
  scale_color_manual(
    values = c("1" = "brown3", "2" = "darkseagreen", "3" = "antiquewhite4"),  # Set colors for the levels
  ) +
  theme_minimal() +   
  theme(
    legend.position = "bottom"
...
Overall, apparent random distribution of residuals. 
...
Diagnostics form predicted residuals show random distribution, although no perfect match with a normal distribution. There is no heteroskedasticity. Relationship likely linear. No evidence of influential outliers. Including observer fixed effect further improved fit and random distribution of residuals. Although not perfect, residuals analysis plus the prior analyses with cardiac output would together suggest that the model is appropriate, although there could possibly be improvements for cardiac index model.
...
```{r}
ggsave(
  filename = file.path(figfolder, "Figure2.tiff"),
  plot = arrangeGrob(fig2A, fig2B, nrow = 2),
  width = 7, height = 10, dpi = 300, bg = "white"
)
```

# Posthoc analysis (low cardiac index)
